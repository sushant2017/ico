...
contract KINGDOMERC20 {
    function totalSupply() public view returns (uint total_Supply);
    function balanceOf (address who) public view returns (uint256);
    function allowance (address KDMOwner, address spender) public view returns (uint);
    function transferFrom (address from, address to, uint value) public returns (bool ok);
    function approve (address spender, uint value) public returns (bool ok);
    function transfer (address to, uint value) public returns (bool ok);
    event    Transfer (address indexed from, address indexed to, uint value);
    event    Approval (address indexed KDMOwner, address indexed spender, uint value);
}


contract KINGDOM is KINGDOMERC20 { 
    
    using SafeMath for uint256;
    
    string  public constant name        = "KINGDOM";                             // Name of the token
    string  public constant symbol      = "KDM";                                // Symbol of token
    uint8   public constant decimals    = 18;
    
    uint    public _totalsupply         = 270000000 * 10 ** 18;                  // 270 million Total Supply
    uint256 maxPublicSale               = 220000000 * 10 ** 18;                  // 220 million Public Sale
    
    uint256 public PricePre             = 7000;                                 // 1 Ether = 7000 tokens in Pre-ICO
    uint256 public PriceICO1            = 4800;                                 // 1 Ether = 4800 tokens in ICO Phase 1
    uint256 public PriceICO2            = 3600;                                 // 1 Ether = 3600 tokens in ICO Phase 2
    uint256 public PublicPrice          = 2800;                                 // 1 Ether = 2800 tokens in Public Sale
    ...
    address public KDMOwner;                                                    // Owner of this contract
    bool stopped = false;
    ...
    function KINGDOM() public {
        KDMOwner            = msg.sender;
        balances[KDMOwner]  = 50000000 * 10 ** 18;                              // 28 million to owner & 20 million to referral bonus
        balances[address(this)] = maxPublicSale;
        stage               = CurrentStages.NOTSTARTED;
        Transfer (0, KDMOwner, balances[KDMOwner]);
        Transfer (0, address(this), balances[address(this)]);
    }
  
    function () public payable {
        require(stage != CurrentStages.ENDED);
        require(!stopped && msg.sender != KDMOwner);
    ...
    function withdrawETH() external onlyOwner {
        KDMOwner.transfer(this.balance);
    }
